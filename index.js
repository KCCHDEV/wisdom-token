...
const fs = require('fs');
const DiscordAuth = require('./auth.js');
const DiscordChecker = require('./checker.js');
...
class DiscordAccountChecker {
    ...
    async checkAccountWithToken(token, identifier) {
        try {
            ...
            console.log(`${'[ERROR]'.red} Unexpected error: ${error.message}`);
        }
    }
    ...
    async bulkCheck() {
        ...
        try {
            ...
            console.log(`${'[ERROR]'.red} Error reading file: ${error.message}`);
        }
    ...
    viewResults() {
        ...
        this.results.forEach((result, index) => {
            console.log(`\n${(index + 1).toString().cyan}. ${result.identifier.yellow}`);
        ...
        if (choice && parseInt(choice) > 0 && parseInt(choice) <= this.results.length) {
            const selectedResult = this.results[parseInt(choice) - 1];
            const checker = new DiscordChecker(selectedResult.token);
            checker.displayAccountInfo(selectedResult.data);
        }
    ...
    saveResults() {
        ...
        try {
            // Prepare data for saving (remove sensitive token info)
            const sanitizedResults = this.results.map(result => ({
                ...
                billing: result.data.billing?.paymentSources?.length > 0,
                friendsCount: result.data.relationships?.friends?.length || 0,
                guildsCount: result.data.guilds?.count || 0
            }));
            
            fs.writeFileSync(filename, JSON.stringify(sanitizedResults, null, 2));
            console.log(`✅ Results saved to ${filename}`.green);
            
        } catch (error) {
            console.log(`${'[ERROR]'.red} Failed to save results: ${error.message}`);
        }
...
# Lines starting with # are ignored`;
    ...
    if (!fs.existsSync('accounts_example.txt')) {
        fs.writeFileSync('accounts_example.txt', exampleContent);
...
module.exports = DiscordAccountChecker;